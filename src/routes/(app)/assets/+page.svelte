...
<form method="GET" class="grid gap-3 sm:grid-cols-[auto_auto_1fr_auto] sm:items-end">
	<div class="space-y-1">
		...
		>
			...
			{#each ASSET_KINDS as k (k)}
				<option value={k} selected={data.filters.kind === k}>{k}</option>
			{/each}
		</select>
	</div>
	<div class="space-y-1">
		...
		>
			...
			{#each data.tags as t (t)}
				<option value={t} selected={data.filters.tag === t}>{t}</option>
			{/each}
		</select>
	</div>
	<div class="space-y-1">
		<Label for="q">Search</Label>
		<Input id="q" name="q" value={data.filters.q} />
	</div>
	...
	<ul class="divide-y divide-border rounded-md border">
		{#each data.assets as a (a.id)}
			<li class="space-y-1 p-4 text-sm">
				...
				<AttrsRenderer defs={data.defs} values={a.custom as Record<string, unknown>} />
			</li>
		{/each}
	</ul>
{/if}
