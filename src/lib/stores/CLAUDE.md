...
- **Hot (in store):** profile, shares, all custom-attr defs (across every table), journal (200 most-recent), sleep (200 most-recent), tasks (all open + recent done), habits + habit_checks (last 90d), categories, budgets, recent transactions (last 90d), assets, projects + project_items, pages list (no blocks).
- **Cold (per-route, on demand):** finance history beyond 90d, page blocks, asset photos, full journal/task/sleep detail rows beyond what's in the list select, AI chat archive, import-batch staging.
