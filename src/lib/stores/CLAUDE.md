...
- **Hot (in store):** profile, shares, all custom-attr defs (across every table), journal (200 most-recent), sleep (200 most-recent), tasks (all open + recent done), habits + habit_checks (last 90d), categories, budgets, recent transactions (last 90d), assets, projects (metadata) + project_blocks (ordered Notion-style rich-text note blocks — replaces the old single body_html, which replaced the old project_items model), datasets metadata (id/name/columns — no row data), shopping_items (active Supplies register), shopping_list_items (the one-shot shopping list), activities + activity_logs (last 90d), recipes (metadata + current ingredients/method bodies — versions load on detail route only), checklists + checklist_items.
