...
- **Hot (in store):** profile, shares, all custom-attr defs (across every table), journal (200 most-recent), sleep (200 most-recent), tasks (all open + recent done), habits + habit_checks (last 90d), categories, budgets, recent transactions (last 90d), assets, projects + project_items, datasets metadata (id/name/columns — no row data), shopping_items (active list), activities + activity_logs (last 90d).
